...
echo [1/7] Kart bilgileri KAP'tan aliniyor (risk degeri, yonetim ucreti, kurucu, denetci)...
call node fetch_kap_fund_info.js --yaz
if errorlevel 1 goto hata

echo [2/7] Net Varlik Tutarlari TEFAS'tan aliniyor...
call node fetch_tefas_net_varlik.js
if errorlevel 1 goto hata

echo [3/7] USD/TRY kuru tazeleniyor...
call node fetch_usdtry.js
if errorlevel 1 goto hata

echo [4/7] 14 fonun fiyat/benchmark serisi arşivden cekiliyor...
for %%F in (AAL AAS AAV AED ANZ AYA DGH JET PKF PKP RTG TLZ URA YLC) do (
  echo    %%F
  call node extract_fund.js %%F
)

echo [5/7] Aylik izgara fonlari (AAL DGH AYA AAV AED TLZ)...
call node build_monthly_data.js
if errorlevel 1 goto hata

echo [6/7] Ozel bloklar: AYA temettu grafigi, ANZ/UANZ grafigi ve Guncel Bilgiler tablosu...
call node extract_aya_dividend.js
call node extract_anz_uanz_chart.js
call node update_anz_guncel_bilgiler.js

echo [7/7] 15 PDF uretiliyor...
