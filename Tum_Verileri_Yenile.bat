...
echo [1/13] Arsiv API'lerden buyutuluyor (TEFAS fiyat + BIST/Nasdaq endeks)...
...
echo [2/13] Kart bilgileri KAP'tan aliniyor (risk degeri, yonetim ucreti, kurucu, denetci)...
call node fetch_kap_fund_info.js --yaz
if errorlevel 1 goto hata

echo [3/13] Portfoy dagilimi TEFAS'tan aliniyor...
call node fetch_tefas_dagilim.js --yaz
if errorlevel 1 goto hata

echo [4/13] USD/TRY kuru tazeleniyor...
call node fetch_usdtry.js
if errorlevel 1 goto hata

echo [5/13] 14 fonun fiyat/benchmark serisi arşivden cekiliyor...
for %%F in (AAL AAS AAV AED ANZ AYA DGH JET PKF PKP RTG TLZ URA YLC) do (
  echo    %%F
  call node extract_fund.js %%F
)

echo [6/13] Aylik izgara fonlari (AAL DGH AYA AAV AED TLZ)...
call node build_monthly_data.js
if errorlevel 1 goto hata

echo [7/13] AAL/AAV/AED/DGH karsilastirma tablosu (Haftalik/Aylik/Yillik/Kurulustan Beri) canli hesaplaniyor...
call node update_comparison_tables.js --yaz
if errorlevel 1 goto hata

echo [8/13] Ozel bloklar: AYA temettu grafigi, ANZ/UANZ grafigi ve Guncel Bilgiler tablosu...
call node extract_aya_dividend.js
call node extract_anz_uanz_chart.js
call node update_anz_guncel_bilgiler.js

echo [9/13] Net Varlik Tutarlari TEFAS'tan aliniyor (UANZ'in rapor tarihi extract_anz_uanz_chart.js'e bagli, o yuzden bu adim en sonda)...
call node fetch_tefas_net_varlik.js
if errorlevel 1 goto hata

echo [10/13] 15 PDF uretiliyor...
call node render_b2.js JET RTG PKF PKP URA
if errorlevel 1 goto hata
call node render_a.js AAL AAS AAV AED ANZ AYA DGH TLZ UANZ YLC
if errorlevel 1 goto hata

echo [11/13] PDF'ler ag klasorundeki tarihli yayin klasorune kopyalaniyor...
call node export_pdfs.js
if errorlevel 1 goto hata

echo [12/13] Tur ozeti: gecen tura gore ne degisti + veri saglik kontrolu...
call node tur_ozeti.js

echo [13/13] Tur ciktisi GitHub'a kaydediliyor...
