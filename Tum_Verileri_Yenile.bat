...
echo [1/11] Arsiv API'lerden buyutuluyor (TEFAS fiyat + BIST/Nasdaq endeks)...
...
echo [2/11] Kart bilgileri KAP'tan aliniyor (risk degeri, yonetim ucreti, kurucu, denetci)...
call node fetch_kap_fund_info.js --yaz
if errorlevel 1 goto hata

echo [3/11] Portfoy dagilimi TEFAS'tan aliniyor...
call node fetch_tefas_dagilim.js --yaz
if errorlevel 1 goto hata

echo [4/11] Net Varlik Tutarlari TEFAS'tan aliniyor...
call node fetch_tefas_net_varlik.js
if errorlevel 1 goto hata

echo [5/11] USD/TRY kuru tazeleniyor...
call node fetch_usdtry.js
if errorlevel 1 goto hata

echo [6/11] 14 fonun fiyat/benchmark serisi arşivden cekiliyor...
for %%F in (AAL AAS AAV AED ANZ AYA DGH JET PKF PKP RTG TLZ URA YLC) do (
  echo    %%F
  call node extract_fund.js %%F
)

echo [7/11] Aylik izgara fonlari (AAL DGH AYA AAV AED TLZ)...
call node build_monthly_data.js
if errorlevel 1 goto hata

echo [8/11] Ozel bloklar: AYA temettu grafigi, ANZ/UANZ grafigi ve Guncel Bilgiler tablosu...
...
echo [11/11] Tur ozeti: gecen tura gore ne degisti + veri saglik kontrolu...
