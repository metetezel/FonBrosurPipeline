...
/**
 * Yerel takvim gunu (YYYY-MM-DD). toISOString() UTC dondurdugu icin Turkiye saatiyle
 * 00:00-03:00 arasinda bir onceki gunu verir - brosur tarihi ve T-1 kesimi bundan
 * etkilenmesin diye yerel tarih kullaniliyor.
 */
...
/**
 * YAYIN TARİHİ — broşürün başlığındaki rozet ve yayın klasörünün adı. Karar (Mete,
 * 29.08.2026): burada PDF'i ürettiğimiz gün yazsın, yani sayfanın yayımlandığı tarih.
 * Verinin tarihi ayrı, arşivde mevcut en taze gün (bkz. lib/arsiv.js `kesimTarihi`);
 * ikisi karışmıyor çünkü bilgi kartındaki "Birim Fiyat (28.08.2026)" satırı kendi
 * tarihini taşıyor. Eski bir seti yeniden üretirken: YAYIN_TARIHI=YYYY-MM-DD.
 */
function yayinTarihi() {
  const iso = process.env.YAYIN_TARIHI || bugunISO();
  return { iso, tr: isoToTR(iso), uzun: isoToTRUzun(iso) };
}

/**
 * VERİ TARİHİ — arşivdeki serinin son günü. Rozette değil; veri eşleştirmesinde
 * kullanılıyor (net varlık hangi güne ait, veri bayat mı).
 */
 ...
 * Rapor tarihi verinin son günü: TEFAS o günü zaten yayınlamışsa aynı gün, henüz
 * yayınlamadıysa bir önceki güne düşer. Gün adını basmak bunun gözden kaçmasını
 * engelliyor.
 */
...
function loadStatic(code) {
  ...
  // Rozet: yayin tarihi (PDF'i urettigimiz gun). Veri tarihi ayri (T-1).
  if (!s.reportDate) s.reportDate = ortak.reportDate || yayinTarihi().tr;
  return s;
}

module.exports = { loadStatic, reportDateFor, yayinTarihi, bugunISO, isoToTR, isoToTRUzun };
