...
function managersHTML(managers) {
  return managers.map(m => `
    ...
    ${m.experience ? `<div class="manager-row" style="margin-top:-2px;">
...
function ctaCardHTML(s) {
  return `
  <div class="card">
    <a href="${s.cta.primaryUrl}" class="cta-btn cta-primary">${s.cta.primary}</a>
    <a href="${s.cta.secondaryUrl}" class="cta-btn cta-secondary">${s.cta.secondary}</a>
    <div class="cta-note">${s.cta.note}</div>
  </div>`;
}

// Donut renk paleti - her iki render motoru da bunu kullaniyor. Pastalar TEFAS
// kirilimina gectikten sonra 5 dilime kadar cikabiliyor; eski iki-renkli/tekrarli
// palette dilimler ayirt edilemiyordu.
const DONUT_PALETTE = [TEAL, ORANGE, '#FFC83E', '#5B9AA8', '#9C4A2F', '#8794A0'];

function chartLegendHTML(s) {
  const hasBench = s.benchmarkAvailable == null || s.benchmarkAvailable.length > 0;
  return `
  <div class="chart-legend">
    <div class="legend-item"><span class="legend-line" style="background:${TEAL}"></span> ${s.fundCode} Endeks (100 ${s.chartCurrency || 'TL'})</div>
